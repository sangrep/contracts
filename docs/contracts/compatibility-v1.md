...
`verify_pack_selection_v1` applies the selection boundary before reuse. Cached reuse requires the
exact current version and unsigned-manifest digest. Rollback requires the same pack, family,
publisher, and channel plus a strictly preceding version inside the current manifest's rollback
range. Both purposes reverify the candidate signature against the current trust registry and
denylist; historical validity is not current authorization.
